...
* Remove outliers in the last data point of the series;
data casuser.&CASTAB._TMP (drop = prev_KPI);
	set casuser.&CASTAB._TMP;
	by Product Month;
	
	retain prev_KPI;
	
	if not first.Product and last.Product then do;
	  if abs(KPI - prev_KPI) > 0.1 then do;
	    *KPI = prev_KPI; * repeats the previous data point;
	    KPI = .; * ignores the data point;
	  end;
	end;
    prev_KPI = KPI;
run;
