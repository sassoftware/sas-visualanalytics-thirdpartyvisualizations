...
;(function(root, factory){
        ...
        query: function(search, conf){
            ...
            if(count > 0 && con.multiple && con.multiLimit == Infinity && con.multiSelectAll){
                ...
                a2.addEventListener("click", function(event){
                    event.preventDefault();
                    var options = self.dropdown.querySelectorAll(".dropdown-inner .dropdown-option"); // modified by Renato: from ".dropdown-option" to ".dropdown-option.selected"
				...
				//this.search.appendChild(li); // by Renato
            }
        ...
        updateLabel: function(label){
            ...
            if(typeof label !== "string"){
                if(c.disabled){
                    ...
                    label = "emptySearch" + (cHAS(this.select, "in-search")? "Search": "");
    ...
    options.prototype = {
        ...
        add: function(key, value, group, selected, disabled, description, rebuild){
            if(key instanceof Object){
				if (key instanceof Array) {
					if (key[0] instanceof Array) {
						for(var k in key){
							this.add(key[k][0], key[k][0], key[k].group, key[k].selected, key[k].disabled, key[k].description, false);
						}
					}
					else {
						for(var k in key){
							this.add(key[k], key[k], key[k].group, key[k].selected, key[k].disabled, key[k].description, false);
						}
					}
				}
				else {
					for(var k in key){
						this.add(key[k].key || k, key[k].value || k, key[k].group, key[k].selected, key[k].disabled, key[k].description, false);
					}
				}
				rebuild = value;
                return (rebuild ? this.self.query() : this.self);
            }
            ...
            // Check Group
			if (typeof group === "boolean") rebuild = group;
